...
export function BeforeAfterSlider({ before, after, alt, height, objectPosition = 'center' }) {
  ...
  return (
    ...
    >
      ...
      <img src={before} alt={`${alt} before`} onLoad={() => setBeforeLoaded(true)} draggable={false}
        style={{ position: 'absolute', inset: 0, width: '100%', height: '100%', objectFit: 'cover', objectPosition, opacity: bothLoaded ? 1 : 0, display: 'block' }} />

      <div style={{ position: 'absolute', inset: 0, width: `${pos}%`, overflow: 'hidden' }}>
        <img src={after} alt={`${alt} after`} onLoad={() => setAfterLoaded(true)} draggable={false}
          style={{ position: 'absolute', inset: 0, width: containerRef.current ? `${containerRef.current.offsetWidth}px` : '100%', height: '100%', objectFit: 'cover', objectPosition, opacity: bothLoaded ? 1 : 0, display: 'block', maxWidth: 'none' }} />
      </div>
